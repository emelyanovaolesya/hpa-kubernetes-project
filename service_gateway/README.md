...
| API Gateway | Service Users | Описание | Авторизация |
|------------|---------------|----------|-------------|
| `POST /v1/auth/register` | `POST /v1/auth/register` | Регистрация | ❌ |
| `POST /v1/auth/login` | `POST /v1/auth/login` | Авторизация | ❌ |
| `GET /v1/users/me` | `GET /v1/users/me` | Текущий профиль | ✅ |
| `PUT /v1/users/me` | `PUT /v1/users/me` | Обновить профиль | ✅ |
| `GET /v1/users/` | `GET /v1/users/all` | Список пользователей | ✅ (admin) |

### Orders Service

| API Gateway | Service Orders | Описание | Авторизация |
|------------|----------------|----------|-------------|
| `POST /v1/orders/` | `POST /v1/orders/create` | Создать заказ | ✅ |
| `GET /v1/orders/` | `GET /v1/orders/my` | Список заказов | ✅ |
| `GET /v1/orders/{id}` | `GET /v1/orders/{id}` | Заказ по ID | ✅ |
| `PUT /v1/orders/{id}/status` | `PUT /v1/orders/{id}/status` | Обновить статус | ✅ |
| `DELETE /v1/orders/{id}` | `DELETE /v1/orders/{id}` | Отменить заказ | ✅ |
...
1. Gateway не хранит данные, он только проксирует запросы
2. JWT токен должен быть одинаковым для всех сервисов (одинаковый SECRET_KEY)
3. Gateway должен запускаться после запуска всех микросервисов
4. Timeout для запросов к микросервисам: 30 секунд
