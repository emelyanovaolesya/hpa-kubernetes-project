...
**⚠️ Важно: запускайте сервисы в следующем порядке!**
...
- 🛡️ Изоляция внутренних сервисов

### Маппинг путей

Gateway трансформирует пути для удобства клиентов:
...
GET  /v1/users/            →  GET  /v1/users/all
POST /v1/orders/           →  POST /v1/orders/create
GET  /v1/orders/           →  GET  /v1/orders/my
GET  /v1/orders/{id}       →  GET  /v1/orders/{id}
PUT  /v1/orders/{id}/status →  PUT  /v1/orders/{id}/status
DELETE /v1/orders/{id}     →  DELETE /v1/orders/{id}
```
...
```bash
# В корне проекта создайте скрипт start_all.sh (Linux/Mac)
#!/bin/bash
cd service_users && python run.py &
cd service_orders && python run.py &
cd service_gateway && python run.py &

# Или start_all.bat (Windows)
start cmd /k "cd service_users && python run.py"
start cmd /k "cd service_orders && python run.py"
start cmd /k "cd service_gateway && python run.py"
```

## 🛑 Остановка сервисов

Нажмите `Ctrl+C` в каждом терминале для остановки соответствующего сервиса.
